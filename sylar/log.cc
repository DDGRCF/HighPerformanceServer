...
LogEvent::LogEvent(const char* file, const int32_t& line,
                   const uint32_t& elapse, const uint32_t& thread_id,
                   const uint32_t& fiber_id, const uint64_t& time)
    : m_file(file),
      m_line(line),
      m_elapse(elapse),
