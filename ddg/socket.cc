...
int64_t Socket::getSendTimeout() {
  FdContext2::ptr ctx = FdMgr::GetInstance()->get(m_sock);
  if (ctx) {
...
bool Socket::init(int sock) {
  FdContext2::ptr ctx = FdMgr::GetInstance()->get(sock);
...
bool Socket::bind(const Address::ptr addr) {
  ...
  if (uaddr) {
    ...
    if (sock->connect(uaddr)) {
      ...
      ::unlink(uaddr->getPath().c_str());  // TODO: 在utils重新定义一个
    }
  }
...
bool Socket::reconnect(uint64_t timeout_ms) {
  if (!m_remoteAddress) {
    DDG_LOG_DEBUG(g_logger) << "reconnect m_remoteAddress is null";
    return false;
  }
...
bool Socket::cancelRead() {
  return IOManager::GetThis()->cancelEvent(m_sock, ddg::IOManager::READ);
}

bool Socket::cancelWrite() {
  return IOManager::GetThis()->cancelEvent(m_sock, ddg::IOManager::WRITE);
}
