...
int32_t FunctionServlet::handle(ddg::http::HttpRequest::ptr request,
                                ddg::http::HttpResponse::ptr response,
                                ddg::http::HttpSession::ptr session) {
  return m_cb(request, response, session);
}

IServletCreator::~IServletCreator() {}

HoldServletCreator::HoldServletCreator(Servlet::ptr slt) : m_servlet(slt) {}

Servlet::ptr HoldServletCreator::get() const {
  return m_servlet;
}

const std::string& HoldServletCreator::getName() const {
  return m_servlet->getName();
}

ServletDispatch::ServletDispatch() : Servlet("ServletDispatch") {
  m_default.reset(new NotFoundServlet("ddg/1.0"));
}

int32_t ServletDispatch::handle(ddg::http::HttpRequest::ptr request,
                                ddg::http::HttpResponse::ptr response,
                                ddg::http::HttpSession::ptr session) {
...
void ServletDispatch::addGlobServlet(const std::string& uri, Servlet::ptr slt) {
  ...
  m_globs.push_back({uri, std::make_shared<HoldServletCreator>(slt)});
}
...
int32_t NotFoundServlet::handle(ddg::http::HttpRequest::ptr request,
                                ddg::http::HttpResponse::ptr response,
                                ddg::http::HttpSession::ptr session) {
  response->setStatus(ddg::http::HttpStatus::NOT_FOUND);
  response->setHeader("Server", "ddg/1.0.0");
